...
const FacultyFDPOrganized = () => {
  ...
  const [type, setType] = useState<'conference' | 'workshop'>('workshop');
  ...
  useEffect(() => {
    if (isDialogOpen) {
      ...
      setMode(editingRecord?.mode || 'online');
      setType(editingRecord?.type || 'workshop');
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (certificateFile && certificateFile.size > 0) data.certificate = certificateFile;
          ...
          <DialogContent>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <Select value={editingRecord?.mode || mode} onValueChange={(v) => setMode(v as 'online' | 'offline' | 'hybrid')}>
                  <SelectTrigger>
              ...
              <div>
                ...
                <Select value={editingRecord?.type || type} onValueChange={(v) => setType(v as 'conference' | 'workshop')}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="rounded-lg border bg-muted/50 px-4 py-3">
                ...
                <p className="mt-1 font-medium">
                  {calculatedDuration || <span className="text-muted-foreground">Select From and To dates</span>}
                </p>
